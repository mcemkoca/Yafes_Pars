...
USING (VALUES
    ...
    (N'RO', N'Roemeens'),
    (N'RU', N'Russisch')
) AS s(language_code, language_label)
ON t.language_code = s.language_code
WHEN MATCHED THEN UPDATE
    SET t.language_label = s.language_label
WHEN NOT MATCHED THEN INSERT(language_code, language_label)
    VALUES(s.language_code, s.language_label);
...
USING (VALUES
    (N'MR',  N'Mijnheer'),
    (N'MRS', N'Mevrouw')
) AS s(title_code, title_label)
ON t.title_code = s.title_code
WHEN MATCHED THEN UPDATE
    SET t.title_label = s.title_label
WHEN NOT MATCHED THEN INSERT(title_code, title_label)
    VALUES(s.title_code, s.title_label);
...
USING (VALUES
    ...
    (N'FAX',      N'Fax'),
    (N'OTHER',    N'Overige')
) AS s(phone_type_code, phone_type_label)
ON t.phone_type_code = s.phone_type_code
WHEN MATCHED THEN UPDATE
    SET t.phone_type_label = s.phone_type_label
WHEN NOT MATCHED THEN INSERT(phone_type_code, phone_type_label)
    VALUES(s.phone_type_code, s.phone_type_label);
...
USING (VALUES
    ...
    (N'TWITTER',   N'Twitter'),
    (N'LINKEDIN',  N'LinkedIn'),
    (N'INSTAGRAM', N'Instagram'),
    (N'OTHER',     N'Andere')
) AS s(social_type_code, social_type_label)
ON t.social_type_code = s.social_type_code
WHEN MATCHED THEN UPDATE
    SET t.social_type_label = s.social_type_label
WHEN NOT MATCHED THEN INSERT(social_type_code, social_type_label)
    VALUES(s.social_type_code, s.social_type_label);

/* ---------- Professionele Statussen ---------- */
MERGE ProfessionalStatus AS t
USING (VALUES
    (N'CIVIL_SERVANT', N'Ambtenaar'),
    (N'WORKER',        N'Arbeider'),
    (N'EMPLOYEE',      N'Bediende'),
    (N'RETIRED',       N'Gepensioneerd'),
    (N'RENTIER',       N'Rentenier'),
    (N'STUDENT',       N'Student'),
    (N'UNEMPLOYED',    N'Werkloos'),
    (N'SELF_EMPLOYED', N'Zelfstandige')
) AS s(professional_status_code, professional_status_label)
ON t.professional_status_code = s.professional_status_code
WHEN MATCHED THEN UPDATE
    SET t.professional_status_label = s.professional_status_label
WHEN NOT MATCHED THEN INSERT(professional_status_code, professional_status_label)
    VALUES(s.professional_status_code, s.professional_status_label);
...
USING (VALUES 
  ...
  (N'OVERIG',     N'Overige/Anders', 1)
) AS src(circumstance_code, circumstance_label, is_active)
ON target.circumstance_code = src.circumstance_code
WHEN MATCHED THEN 
  UPDATE SET circumstance_label = src.circumstance_label, is_active = src.is_active
WHEN NOT MATCHED THEN 
  INSERT (circumstance_code, circumstance_label, is_active)
  VALUES (src.circumstance_code, src.circumstance_label, src.is_active);

/* ---------- ClaimPaymentMethod (mogelijkheden voor betaalwijze) ---------- */
...
) AS source (circumstance_code, circumstance_label, is_active)
ON target.circumstance_code = source.circumstance_code
WHEN MATCHED THEN UPDATE SET target.circumstance_label = source.circumstance_label, target.is_active = source.is_active
WHEN NOT MATCHED THEN INSERT (circumstance_code, circumstance_label, is_active) VALUES (source.circumstance_code, source.circumstance_label, source.is_active);
