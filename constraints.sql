...
-- This script assumes all tables are created (see 01_schema.sql).
...
ALTER TABLE NaturalPerson
    ADD CONSTRAINT CK_NaturalPerson_Lifespan CHECK (death_date IS NULL OR birth_date IS NULL OR death_date >= birth_date);
...
ALTER TABLE Claim_Circumstance
    ...
    ADD CONSTRAINT FK_ClaimCircumstance_CircumstanceType FOREIGN KEY (claim_circumstance_type_code) REFERENCES ClaimCircumstanceType(circumstance_code);
