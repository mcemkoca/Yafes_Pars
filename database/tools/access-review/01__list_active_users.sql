...
-- Read-only. Run against TEST or PROD with appropriate credentials.
...
SELECT
    ...
    u.last_login_utc                        AS last_login_utc,
    DATEDIFF(DAY, u.last_login_utc, SYSUTCDATETIME()) AS days_since_last_login
FROM core.AppUser u
...
SELECT
    ...
    u.display_name,
    u.last_login_utc,
    DATEDIFF(DAY, u.last_login_utc, SYSUTCDATETIME()) AS days_inactive
FROM core.AppUser u
INNER JOIN core.Tenant t ON t.tenant_id = u.tenant_id
WHERE u.is_active = 1
  AND (u.last_login_utc IS NULL OR u.last_login_utc < DATEADD(DAY, -90, SYSUTCDATETIME()))
