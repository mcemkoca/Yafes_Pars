...
public sealed class EmailTools
{
    ...
    [McpServerTool, Description(
        "Verstuur een herinneringsmail voor openstaande/vervallen facturen (OVERDUE). / Vadesi gecmis faturalar icin hatirlatma e-postasi gonder.\n" +
    ...
    {
        // Email via person.Person (tenant_id) -> person.Email (is_primary=1)
        ...
        foreach (var inv in invoices)
        {
            ...
            var html    = BuildOverdueHtml(inv, recipientName);
            var msg     = new EmailMessage(inv.RecipientEmail, recipientName, subject, html);
    ...
    [McpServerTool, Description(
        "Verstuur een betalingsbevestiging voor een betaalde factuur. / Odenen fatura icin odeme onayi e-postasi gonder.")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var html    = $"""
            ...
            <p>Wij bevestigen ontvangst van uw betaling van <strong>&euro;{amount:F2}</strong> voor factuur {invoiceId:D}.</p>
    ...
    [McpServerTool, Description(
        "Verstuur een verlengingsherinnering voor een polis die binnenkort afloopt. / Yakinda sona erecek police icin yenileme hatirlatmasi gonder.")]
    ...
    [McpServerTool, Description(
        "Haal de e-maillog op voor audit en compliance. / Denetim ve uyumluluk icin e-posta gunlugunu getir.")]
    ...
    private static string BuildOverdueHtml(OverdueInvoiceRow inv, string recipientName) => $"""
        <p>Beste {recipientName},</p>
        <p>Wij wijzen u erop dat factuur {inv.InvoiceId:D} van <strong>&euro;{inv.Amount:F2}</strong>
